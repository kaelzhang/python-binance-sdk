...
- ``eventStreamTerminated`` — DROPPED Round-8 L-2 (2026-05-30).  The futures
  user-data-streams docs do NOT list this event (only the Spot user-data-stream
  page does):
  ...
  Per the strict docs-only policy the ``FuturesEventStreamTerminatedHandlerBase``
  class and the ``EVENT_STREAM_TERMINATED`` entry in
  ``FuturesUserProcessor.PAYLOAD_TYPES`` were removed in the same commit.
  The Spot ``EventStreamTerminatedHandlerBase`` is unaffected — that surface
  remains because Spot docs do list the event.  For the futures dedicated
  user-data fstream, the SDK still treats ``listenKeyExpired`` as the
  documented recovery channel; subscribe to
